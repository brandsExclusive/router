...
const handle = (
  ...
  { url, operationId, preHandlers, handlers, schema, isPublic, tags, summary, description, warnOnRequestValidationError, mutateQuery, mutateParams }
) => {
  let _handlers = []
  ...
  if (schema && schema.request) {
    _handlers.push(requestValidator({ logger, schema: schema.request, warnOnRequestValidationError, mutateParams, mutateQuery }))
  }
